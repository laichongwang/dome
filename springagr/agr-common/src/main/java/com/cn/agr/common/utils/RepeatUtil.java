...
public class RepeatUtil {
    ...
    public RepeatUtil(byte[] hashValue) {
    ...
    public RepeatUtil(String hashValue) {
        this(toBytes(hashValue));
    }

    public RepeatUtil(BufferedImage src) {
        this(hashValue(src));
    }
    ...
    /**
     * 从压缩格式指纹创建{@link RepeatUtil}对象
     * @param compactValue
     * @return
     */
    public static RepeatUtil createFromCompact(byte[] compactValue) {
        return new RepeatUtil(uncompact(compactValue));
    }
    ...
    public boolean equals(Object obj) {
        if (obj instanceof RepeatUtil) {
            return Arrays.equals(this.binaryzationMatrix, ((RepeatUtil) obj).binaryzationMatrix);
        } else
    ...
    public float compare(String hashValue) {
        return compare(new RepeatUtil(hashValue));
    }
    ...
    public float compare(BufferedImage image2) {
        return compare(new RepeatUtil(image2));
    }
     ...
     */
    public float compare(RepeatUtil src) {
    ...
    private static float compare(byte[] f1, byte[] f2) {
        if (f1.length != f2.length)
            throw new IllegalArgumentException("mismatch RepeatUtil length");
        int sameCount = 0;
        for (int i = 0; i < f1.length; ++i) {
            if (f1[i] == f2[i])
                ++sameCount;
        }
        System.out.println("count:" + sameCount + ",leng:" + f1.length);
    ...
    public static float compare(BufferedImage image1, BufferedImage image2) {
        return new RepeatUtil(image1).compare(new RepeatUtil(image2));
    }

    public static void main(String[] agr) {
        try {
            float reuslt = RepeatUtil.compare(ImageIO.read(new File("L:\\image\\cao.jpg")), ImageIO.read(new File("L:\\image\\cao4.jpg")));
            System.out.println("相似度：" + reuslt);
            // imgNewFormat();
        } catch (Exception e) {
     ...
     * 装换图片格式
     */
    public static void imgToNewFormat() {
        BufferedImage bufferedImage;
        try {
            // 1.读取图片
            InputStream is = new FileInputStream(new File("L:\\image\\a.tif"));
            bufferedImage = ImageIO.read(is);
            // 2.创建一个空白大小相同的RGB背景
            BufferedImage newBufferedImage = new BufferedImage(bufferedImage.getWidth(), bufferedImage.getHeight(), BufferedImage.TYPE_INT_RGB);
            newBufferedImage.createGraphics().drawImage(bufferedImage, 0, 0, Color.WHITE, null);
            // 3.再次写入的时候以jpeg图片格式
            ImageIO.write(newBufferedImage, "jpg", new File("L:\\image\\new.tif"));
     ...
     * 特殊图片格式装换
     */
    public static void imgNewFormat() {
        // tif转换到jpg格式
        try {
            String input2 = "L:\\image\\btif.tif";
            String output2 = "L:\\image\\bjpg.jpg";
